...
This document describes how to monitor the Privacy-Shield Payments system —
the Soroban shield contract, the indexer, and the frontend API — and the
...
1. **Soroban RPC liveness** — `getLatestLedger` succeeds and the latest
   ledger is fresh.
2. **Contract-event stall** — the shield contract has emitted events in the
   configured window (indexer stall detection). Requires `CONTRACT_ID`.
3. **Frontend API health** — optional, requires `FRONTEND_HEALTH_URL`.
...
|---|---|---|---|---|
| RPC reachability | `getLatestLedger` HTTP/parse result | — | RPC unreachable or invalid response | `RPC_URL=https://soroban-testnet.stellar.org` |
| Ledger freshness | `now - latest.closeTime` | — | `> LEDGER_STALE_SECONDS` | `LEDGER_STALE_SECONDS=300` (5 min) |
...
| Frontend health | `/health` HTTP 2xx | — | non-2xx / unreachable | `FRONTEND_HEALTH_URL` (unset) |
| Webhook delivery | HTTP 2xx from webhook | delivery failure logged | — | `WEBHOOK_URL` (unset) |

Suggested thresholds for production (mainnet):

| Check | Warn | Critical | Rationale |
|---|---|---|---|
| Ledger staleness | — | > 5 min | Testnet produces ~1 ledger / 5 s; a 5-minute gap means the RPC or indexer is stalled. |
| Event stall window | — | 60 min with zero events | Long enough to ignore quiet hours; short enough to catch a dead indexer. Tune per traffic: high-volume networks may use 15 min. |
| RPC response timeout | — | 10 s | `--max-time 10` per request in `monitor.sh`. |
...
- **WARN** (`exit 2`): a threshold is crossed but the system is still
  answering — e.g. a quiet window with no contract events. Logged, no
  webhook by default.
- **CRITICAL** (`exit 1`): the system is broken or unreachable — RPC down,
...
- The event-stall check counts `contract`-type events for `CONTRACT_ID` over
  ...
  ~1 ledger / 5 s ≈ 12 / min). If the network cadence differs, adjust
  `LEDGERS_PER_MINUTE`.
