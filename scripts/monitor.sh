#!/usr/bin/env bash
#
# Privacy-Shield Payments — indexer & system health monitor.
#
...
if (( ledger_age > LEDGER_STALE_SECONDS )); then
    alert CRITICAL "latest ledger $latest_seq is $ledger_age s old (threshold ${LEDGER_STALE_SECONDS}s) — indexer/RPC stall"
    status=1
else
...
if [[ -n "$CONTRACT_ID" ]]; then
    ...
    if [[ -z "$events_resp" ]]; then
        ...
        if [[ "$event_count" == "-1" ]]; then
            ...
            alert WARN "no contract events in last ${STALL_WINDOW_MINUTES}m (ledgers ${start_ledger}..${latest_seq}) — possible indexer stall"
            status=2
        else
